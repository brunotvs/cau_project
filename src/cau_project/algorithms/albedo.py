        ...
        albedo: ee.Image = (
            ...
            .map(
                lambda img: img.expression(
                    ...
                    {
                        "red": img.select("B4").unitScale(0, PIXEL_MAX),
                        "blue": img.select("B2").unitScale(0, PIXEL_MAX),
                        "nir": img.select("B8").unitScale(0, PIXEL_MAX),
                        "swir": img.select("B11").unitScale(0, PIXEL_MAX),
                        "swir2": img.select("B12").unitScale(0, PIXEL_MAX),
                    },
                )
            )
            .median()
